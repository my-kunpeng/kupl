...
typedef struct kupl_sched_expand_static_mq {
    int max_num_executors;
    int num_executors; // num executors
    int queue_size;    // queue size
    priority_queue_t *priority_queues;
    queue_t **queues; // local_queues own by executors
} kupl_sched_expand_static_mq_t;

static int enable_priority;

static int kupl_sched_expand_static_mq_init(kupl_sched_plugin_property_t *property)
{
...
static void kupl_sched_expand_static_mq_fini() {}

static void kupl_sched_expand_static_mq_cleanup(void *_sched);
static void *kupl_sched_expand_static_mq_create()
{
    auto host_info = kupl_get_host_info();
    int num_executors = 1;
    int max_num_executors = host_info->pu_cnt;

    kupl_sched_expand_static_mq_t *sched =
        (kupl_sched_expand_static_mq_t *)kupl_calloc(1, sizeof(kupl_sched_expand_static_mq_t));
    if (kupl_unlikely(sched == nullptr)) {
        return nullptr;
    }
    sched->num_executors = num_executors;
    sched->max_num_executors = max_num_executors;
    sched->queue_size = kupl_config_get_value(KUPL_SCHED_QUEUE_LENGTH);
    sched->queues = (queue_t **)kupl_calloc((size_t)max_num_executors, sizeof(queue_t *));
    ...
    return sched;
err:
    kupl_warn("kupl_sched_expand_static_mq_create failed");
    kupl_sched_expand_static_mq_cleanup(sched);
    return nullptr;
}

static void *kupl_sched_expand_static_mq_expand(void *sched)
{
    kupl_sched_expand_static_mq_t *sched_mq = (kupl_sched_expand_static_mq_t *)sched;

    if (sched_mq->num_executors + 1 > sched_mq->max_num_executors) {
        kupl_warn("kupl_sched_expand_static_mq_expand failed");
        kupl_sched_expand_static_mq_cleanup(sched_mq);
        return nullptr;
    }
    sched_mq->queues[sched_mq->num_executors] = queue_create(sched_mq->queue_size);
    if (kupl_unlikely(sched_mq->queues[sched_mq->num_executors] == nullptr)) {
        kupl_warn("kupl_sched_expand_static_mq_expand failed");
        kupl_sched_expand_static_mq_cleanup(sched_mq);
        return nullptr;
    }
    sched_mq->num_executors++;
    return sched_mq;
}

static void kupl_sched_expand_static_mq_cleanup(void *_sched)
{
    kupl_sched_expand_static_mq_t *sched = (kupl_sched_expand_static_mq_t *)_sched;
    if (kupl_unlikely(sched == nullptr)) {
        return;
    }
...
static int kupl_sched_expand_static_mq_add_tb(void *_sched, kupl_taskbase_t *tb)
{
    kupl_sched_expand_static_mq_t *sched = (kupl_sched_expand_static_mq_t *)_sched;
    ...
    if (kupl_unlikely(eid != geid)) {
        return kupl_log_error_return(ERROR, "can't add tb to other threads on expanded sched");
    }
    // priority_queue
    if (enable_priority && (tb->flag & KUPL_TB_FLAG_PRIORITY)) {
        return priority_queue_add_tb(sched->priority_queues, tb);
    }
    eid = kupl_get_local_executor_num(eid);

    return queue_add_tb(sched->queues[eid], tb);
}

static kupl_taskbase_t *kupl_sched_expand_static_mq_get_tb(void *_sched, kupl_compute_place_t cp)
{
    kupl_sched_expand_static_mq_t *sched = (kupl_sched_expand_static_mq_t *)_sched;
    (void)cp;
...
static const kupl_sched_plugin_api_t KUPL_SCHED_PLUGIN_GLOBAL_VAR(expand_static_mq) = {
    .name = KUPL_SCHED_PLUGIN_EXPAND_STATIC_MQ_NAME,
    .init = kupl_sched_expand_static_mq_init,
    .fini = kupl_sched_expand_static_mq_fini,
    .create = kupl_sched_expand_static_mq_create,
    .expand = kupl_sched_expand_static_mq_expand,
    .cleanup = kupl_sched_expand_static_mq_cleanup,
    .add_tb = kupl_sched_expand_static_mq_add_tb,
    .get_tb = kupl_sched_expand_static_mq_get_tb,
};

const kupl_sched_plugin_api_t *kupl_sched_expand_static_mq_get_instance()
{
    return &KUPL_SCHED_PLUGIN_GLOBAL_VAR(expand_static_mq);
}