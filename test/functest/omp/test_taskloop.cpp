...
TEST_F(test_taskloop, invalid_desc)
{
    ...
    KUPL_1D_RANGE_INIT(invalid_range, 0, loop_tasks);
    invalid_range.dim = 0;
    ret = kupl_graph_submit(graph_, &info);
    ASSERT_TRUE(ret == KUPL_ERROR);

    KUPL_1D_RANGE_INIT(invalid_range, 0, loop_tasks);
    invalid_range.nd_range[0].step = 0;
    ret = kupl_graph_submit(graph_, &info);
    ASSERT_TRUE(ret == KUPL_ERROR);

    KUPL_1D_RANGE_INIT(invalid_range, 0, loop_tasks);
    invalid_range.nd_range[0].lower = loop_tasks + 1;
    ret = kupl_graph_submit(graph_, &info);
    ASSERT_TRUE(ret == KUPL_ERROR);
}
    ...
    for (int i = 0; i < range_1d_len; i++) {
        for (int j = 0; j < blocksizes_len; j++) {
            #pragma omp parallel
            {
                ...
                kupl_egroup_barrier(egroup_);
            }
            EXPECT_EQ(count.load(), range_1d[i]);
        }
    }
}
    ...
    for (int i = 0; i < range_2d_len; i++) {
        for (int j = 0; j < blocksizes_len; j++) {
            #pragma omp parallel
            {
                ...
                if (tid == 0) {
                    count.store(0);
                    KUPL_STRIDE_2D_RANGE_INIT(range, 0, range_2d[i][0], 1, blocksizes[j],
                                              0, range_2d[i][1], 1, blocksizes[j]);
                    int ret = kupl_graph_submit(graph_, &info);
                    EXPECT_EQ(ret, KUPL_OK);
                    kupl_graph_wait(graph_);
                }
            ...
            EXPECT_EQ(count.load(), range_2d[i][0] * range_2d[i][1]);
        }
    }
}
...
TEST_F(test_taskloop, taskloop_3d)
{
    ...
    for (int i = 0; i < range_3d_len; i++) {
        for (int j = 0; j < blocksizes_len; j++) {
            ...
            EXPECT_EQ(count.load(), range_3d[i][DIM_0] * range_3d[i][DIM_1] * range_3d[i][DIM_2]);
        }
    }
}
