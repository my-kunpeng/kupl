...
#define RANGE_RANK_MIN INT32_MIN
#define RANGE_RANK_MAX INT32_MAX
#define RANGE_IS_NULL 1
#define RANGE_NOT_NULL 0
...
void win_barrier_example(int test_count)
{
    printf("start -- %s\n", __func__);
    ...
    DT_FUZZ_START(0, test_count, (char *)__func__, 0)
    {
        ...
        kupl_shm_peer_fence(win, remote_rank);
        if (win) {
            kupl_shm_win_free(win);
        }
