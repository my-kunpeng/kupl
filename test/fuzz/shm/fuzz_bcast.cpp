...
static const int RANGE_COUNT_MAX = 1000000;
static const int RANGE_DATATYPE_MIN = 0;
static const int RANGE_DATATYPE_MAX = 0;
...
void bcast_example(int test_count)
{
    printf("start -- %s\n", __func__);
    ...
    {
        int ret = kupl_shm_comm_create(numprocs, myid, getpid(), oob_cbs_h, (void *)comm, &kupl_comm);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_comm_create failed\n");
        }

        int count = *(int *)DT_SetGetNumberRange(&g_Element[0], RANGE_COUNT_MIN, RANGE_COUNT_MIN, RANGE_COUNT_MAX);
        int datatype_idx =
            *(int *)DT_SetGetNumberRange(&g_Element[1], RANGE_DATATYPE_MIN, RANGE_DATATYPE_MIN, RANGE_DATATYPE_MAX);
        int root = *(int *)DT_SetGetNumberRange(&g_Element[2], 0, 0, numprocs - 1);
        ...
        ret = kupl_shm_win_alloc(buf_size, kupl_comm, &win_alloc_buffer, &win);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_win_alloc failed\n");
        }
        ret = kupl_shm_bcast_init(win_alloc_buffer, count, datatype, root, kupl_comm, &request);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_bcast_init failed\n");
        }
        ret = kupl_shm_request_start(request);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_request_start failed\n");
        }
        kupl_shm_request_wait(request);
        ret = kupl_shm_request_free(request);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_request_start failed\n");
        }
        ret = kupl_shm_win_free(win);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_request_start failed\n");
        }
        ret = kupl_shm_comm_destroy(kupl_comm);
        if (ret != KUPL_OK) {
            fprintf(stderr, "kupl_shm_comm_destroy failed\n");
        }
    }
    DT_FUZZ_END();
    printf("end -- %s\n", __func__);
