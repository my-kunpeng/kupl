...
static const int RANGE_OP_DEFAULT = 2;
static const int RANGE_OP_MIN = 2;
static const int RANGE_OP_MAX = 2;

static const kupl_shm_datatype_t datatype_list[] = {
...
static const kupl_shm_reduce_op op_list[] = {
    KUPL_SHM_REDUCE_OP_MAX,
...
void allreduce_example(int test_count)
{
    printf("start -- %s\n", __func__);
    ...
    DT_FUZZ_START(0, test_count, (char *)__func__, 0)
    {
        ...
        int op_idx = *(int *)DT_SetGetNumberRange(&g_Element[cnt++], RANGE_OP_DEFAULT, RANGE_OP_MIN, RANGE_OP_MAX);
        MPI_Bcast(&count, 1, MPI_INT, 0, comm);
        MPI_Bcast(&datatype_idx, 1, MPI_INT, 0, comm);
        MPI_Bcast(&data, 1, MPI_INT, 0, comm);
        MPI_Bcast(&op_idx, 1, MPI_INT, 0, comm);
        ...
        if (ret_comm == KUPL_OK) {
            ...
            int ret_send = kupl_shm_win_alloc(buf_size, kupl_comm, &sendbuf, &send_win);
            int ret_recv = kupl_shm_win_alloc(buf_size, kupl_comm, &recvbuf, &recv_win);
            if (ret_send == KUPL_OK) {
                set_buffer(sendbuf, count, data, datatype);
            }

            if (ret_send == KUPL_OK && ret_recv == KUPL_OK) {
                kupl_shm_reduce_op op = op_list[op_idx];
                int ret = kupl_shm_allreduce_init(sendbuf, recvbuf, count, datatype, op, kupl_comm, &request);
                if (ret == KUPL_OK) {
